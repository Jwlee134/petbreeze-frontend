...
export interface MissingReportSecondGoBack {
  goBack: () => void;
}
...
const MissingReportSecondPage = forwardRef<MissingReportSecondGoBack, Props>(
  ({ navigation, deviceID, isModify }, ref) => {
    ...
    const [register, { isLoading: loadingRegister }] =
      deviceApi.usePostMissingReportMutation();
    const [updateAvatar, { isLoading: loadingAvatar }] =
      deviceApi.useUpdateMissingReportThumbnailMutation();
    const [update, { isLoading: loadingUpdate }] =
      deviceApi.useUpdateMissingReportMutation();
...
export default MissingReportSecondPage;
